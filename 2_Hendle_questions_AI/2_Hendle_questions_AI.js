...
const { trainingdata, parttwo, lablefor2ndpart, lengthofdata } = require('./2_Hendle_questions_AI_Data');
const { nameofAI_2 } = require('../AI_Route/AI_Routes');
const fs = require('fs');
...
function createJSFileInFolder(trainagain) {
    ...
    if (trainagain) { //it will train only when we will pass false when calling the function
        ...
        checkTrainedModel('What is the most efficient method for inspecting spline quality?');
        ...
        const nameofaifromtrainingdata = nameofAI_2(); //name of the txt file that will be saves as trained model
...
function readTextFile(filePath, nameoftxt) {
    ...
    try {
        ...
        .then(data => {
            ...
            document.getElementById('AI_Model_txt2').innerHTML = data;
...
const nameofaifromtrainingdata = nameofAI_2();
...
if ( !parseInt(condi) == 1) {
    ...
    //const fileContent = document.getElementById('AI_Model_txt2').innerHTML; //uncomment this when making bundle
    ...
    function brainshit() {
        ...
        //Getting the biggest intension____________________________________________    //NEED CHANGES HERE WHEN MAKING NEW AI +, FOR GETTING THE HIGESTING VALUE AND FOR WHICH OUTPUT WORD

        function getbiggest() { //gets the name of the variable that as bihest value

            for (let q = 0; q < respo.length; q++) {
                for (let t = 0; t < respo.length; t++) {
                    ...
                    if (tempcounter == (respo.length - 1)) {
                        return parttwoforindex2[q];
                    }
